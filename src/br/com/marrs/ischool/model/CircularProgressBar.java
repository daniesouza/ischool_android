...
package br.com.marrs.ischool.model;
...
public class CircularProgressBar extends ProgressBar{
	...
	public void init(AttributeSet attrs, int style){
		...
		if(color==null)
			mProgressColorPaint.setColor(res.getColor(R.styleable.CircularProgressBar_progressColor));
		else
			mProgressColorPaint.setColor(Color.parseColor(color));

		color = a.getString(R.styleable.CircularProgressBar_defaultBackground);
		if(color==null)
			mBackgroundColorPaint.setColor(res.getColor(R.styleable.CircularProgressBar_defaultBackground));
		else
			mBackgroundColorPaint.setColor(Color.parseColor(color));

		color = a.getString(R.styleable.CircularProgressBar_titleColor);
		if(color==null)
			mTitlePaint.setColor(res.getColor(R.styleable.CircularProgressBar_titleColor));
		else
			mTitlePaint.setColor(Color.parseColor(color));

		color = a.getString(R.styleable.CircularProgressBar_subtitleColor);
		if(color==null)
			mSubtitlePaint.setColor(res.getColor(R.styleable.CircularProgressBar_subtitleColor));
		else
			mSubtitlePaint.setColor(Color.parseColor(color));


		String t = a.getString(R.styleable.CircularProgressBar_titleProgress);
		if(t!=null)
			mTitle = t;

		t = a.getString(R.styleable.CircularProgressBar_subtitleProgress);
		if(t!=null)
			mSubTitle = t;

		mStrokeWidth = (int) ImagemUtils.convertDpToPixel(a.getFloat(R.styleable.CircularProgressBar_strokeWidth, STROKE_WIDTH),this.getContext());
		...
		mTitlePaint.setTextSize(ImagemUtils.convertDpToPixel(a.getFloat(R.styleable.CircularProgressBar_font_title_size, 12.0f),this.getContext())); 
		...
		mSubtitlePaint.setTextSize(ImagemUtils.convertDpToPixel(a.getFloat(R.styleable.CircularProgressBar_font_subtitle_size, 12.0f),this.getContext())); 
	...
	public synchronized void setTitleProgress(String titleProgress){
		this.mTitle = titleProgress;
		invalidate();
	}

	public synchronized void setSubTitleProgress(String subtitleProgress){
		this.mSubTitle = subtitleProgress;
		invalidate();
	}
